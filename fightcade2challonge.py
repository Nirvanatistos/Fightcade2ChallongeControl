...
LANGUAGES = {
    "es": {
        "title": "FightCade2Challonge Control v1.0",
        "connect": "Conectar",
        "user": "Usuario:",
        "api_key": "API Key:",
...
class ChallongeScoreboardApp:
    def __init__(self, root):
        self.root = root
        self.root.title("FightCade2Challonge Control v1.0")
    ...
    def create_widgets(self):
        ...
        self.player1_name = self.canvas.create_text(
            450.0,
            70.0,
            anchor="center",  # ← Esta línea es clave
            text="Nick Player1",
            fill="#FFFFFF",
            font=("Arial Black", 12, "bold"),
            justify="center",
        ...
        self.losers_scroll_x.place(x=0, y=207, width=1346, height=15)  # Ajustado para que se vea bien
    ...
    def show_notification(self, message, duration=2000, permanent=False):
        ...
        if hasattr(self, "_notification_after_id") and self._notification_after_id is not None:
            try:
                ...
                pass  # Si el after_id ya no es válido, lo ignoramos
        ...
        if not permanent:
            ...
            self._notification_after_id = None  # No programamos limpieza
    ...
    def load_matches(self, event=None):
        ...
        try:
            ...
            if match_list:
                ...
                self.combobox_match.set("")  # Dejar vacío para que el usuario elija
                self.notification_label.config(text="Elija Partido!", fg="#FFFFFF")  # Opcional: color diferente
            else:
                self.combobox_match.config(state="disabled")
                self.combobox_match.set("")
                self.notification_label.config(text="No hay Matches disponibles.")

            self.draw_bracket()  # Dibujar bracket después de cargar partidos
    ...
    def display_horizontal_bracket(self, container, rounds_dict, participants, is_losers=False):
        ...
        special_keys = [1000, 1001, 1002]
        for key in special_keys:
            if key in sorted_rounds:
                sorted_rounds.remove(key)
                sorted_rounds.append(key)

        for key in sorted_rounds:
            matches_in_round = rounds_dict[key]

            # Generar título de ronda
            if key in [1000, 1001, 1002]:
                title = {
                    1000: "Semifinales",
                    1001: "Final",
                    1002: "Grand Final"
    ...
    def get_round_description(self, round_num, total_participants):
        ...
        if round_num < 0:
            ...
            try:
                ...
                if losers_rounds and abs_rnd == max(losers_rounds):
                    return "Losers Final"
                else:
                    return f"Loser Round {abs_rnd}"
            except Exception as e:
        ...
        try:
            ...
            # En caso de error, usar total_participants como fallback
    ...
    def on_click_match(self, match, participants, label, is_losers=False):
        ...
        # ← Aquí va la parte nueva ↓↓↓
        self.update_next_match_combobox()  # Llenar el combobox_next_match
        self.update_match_combobox()      # Refrescar el combobox_match
        ...
        self.auto_update_enabled = True
        if hasattr(self, "button_submit"):
            self.button_submit.config(state="disabled")  # ← Resetear botón submit
    ...
    def add_point_player1(self):
        ...
        if hasattr(self, "button_submit"):
            self.button_submit.config(state="normal")  # ← Aquí está el cambio
    ...
    def toggle_manual_mode_warning(self):
        ...
        if response:
            self.auto_update_enabled = True  # Modo manual activado
            if hasattr(self, "button_submit"):
                self.button_submit.config(state="normal")
        else:
    ...
    def load_selected_match(self, event=None):
        ...
        try:
            ...
            # ← Llama a update_next_match_combobox ↓↓↓
            ...
            self.auto_update_enabled = True
            if hasattr(self, "button_submit"):
                self.button_submit.config(state="disabled")  # ← Botón submit siempre deshabilitado al inicio
    ...
    def browse_directory(self):
        ...
        # ← Opcional: refrescar puntajes si es posible
        try:
            score1, score2 = self.read_scores_from_files()
            if score1 is not None and score2 is not None:
                self.score1.set(score1)
    ...
    def reopen_match(self):
        ...
        try:
            ...
            self.load_matches()  # ← Esto es clave
            self.update_match_combobox()
    ...
    def auto_update_scores(self):
        # ← Aquí agregamos la condición nueva ↓↓↓
        if not self.loaded_match or not self.auto_update_enabled or self.mode_button["text"] != "Modo FightCade":
            return
        ...
        if score1 is None or score2 is None:
            print("No se pudieron leer los puntajes.")
    ...
    def is_special_match(self, match_data):
        ...
        try:
            ...
            losers_matches = [m for m in matches if m["round"] < 0]
            # Obtener todos los partidos del Winners Bracket (round > 0)
            winners_matches = [m for m in matches if m["round"] > 0]

            # Último partido del Winners Bracket
            last_winners_match = max(winners_matches, key=lambda m: m["round"], default=None)

            # Último partido del Losers Bracket
            last_losers_match = max(losers_matches, key=lambda m: abs(m["round"]), default=None)

            # Si es final de Winners Bracket
            if last_winners_match and match_data["id"] == last_winners_match["id"]:
                return "winners_final"

            # Si es final de Losers Bracket
            elif last_losers_match and match_data["id"] == last_losers_match["id"]:
                return "losers_final"

            # Si tiene group_id → probablemente sea Grand Final
            elif match_data.get("group_id") is not None:
                return "grand_final"
    ...
    def get_ft_value(self):
        ...
        try:
            ...
            match_type = self.is_special_match(match_data)

            # Usar FT Final para partidos especiales
            if match_type in ["winners_final", "losers_final", "grand_final"]:
                try:
    ...
    def check_auto_submit_result(self, score1, score2):
        ...
        if match_type in ["losers_final", "winners_final", "grand_final"]:
            try:
                ft_value = int(self.combo_ft_final.get()[2:])  # Ej: "FT3" → 3
            except:
                ft_value = 3
    ...
    def on_reset_scores_click(self):
        ...
        if self.auto_update_id is not None:
            try:
                ...
                pass
            self.auto_update_id = None  # Limpiamos la referencia después de cancelar
